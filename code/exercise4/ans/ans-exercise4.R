...
fishdata <- read.csv("fish.CSV", fileEncoding = "UTF-8-BOM")
fishdata

# 接下來我們要使用手刻的最小方差法，來回歸 fishdata 中的 weights 屬性
# 我們要以 Weight 以外的屬性來回歸 Weight
X <- model.matrix(~Species+Length1+Length2+Length3+Height+Width, data = fishdata)
...
g <- lm(Weight~Species+Length1+Length2+Length3+Height+Width, fishdata)
...
my.corr_2 <- cor(g$fitted.value, y)
my.r2_2 <- my.corr_2^2
my.rmse_2 <- sqrt(mean((g$fitted.value-y)^2))
